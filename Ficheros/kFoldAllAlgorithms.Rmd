...
prediction.predicted <- function(predicted, q = default_q) {
    ...
    nElems <- length(predicted) / 2
    
    # Use foreach to parallelize the loop
    counts <- foreach(i = 1:nElems, .combine = "c") %dopar% {
      lapply(predicted[[2 * i]], function(x) as.integer(x == OUTPUT_MAJ))
    }
    sum_vector <- colSums(do.call(rbind, counts))
    
    # Stop the cluster and unregister it
    stopCluster(cl)
    pred <- ifelse(sum_vector / nElems < q, OUTPUT_MIN, OUTPUT_MAJ)
    return(pred)
}
...
train_IPIP <- function( prop.maj, OUTPUT, min_str, maj_str, train.set, test.set, 
  ...
  # #Full maj class approach
  # vals = 1:nmaj
  # p=0
  # while(length(vals)>majSubSize){
  #   p = p+1
  #   id.minoritary <- sample(x = 1:nmin, size = minSubSize) #Index for minoritary class for each subset
  #   id.majoritary <- sample(vals, size = majSubSize, replace = FALSE) #Indexes for majoritary class for each subset
  #    
  #   vals <- vals[!(vals %in% id.majoritary)]
  #   dfs[[p]] <- rbind(minoritary[id.minoritary,],majoritary[id.majoritary,])
  # 
  # }
  ...
  for(k in 1:p){
    ...
    while(length(Ek)<=b && i<mt(b,length(Ek))){
      ...
      cat(sprintf("Training element %d in %d possible values\n", i+1,mt(b,length(Ek))))
      model <- configuration[[length(Ek)+1]](df[ind.train,], metrics, OUTPUT, CHOSEN_METRIC)
...
powerset = function(s, test.set){
    ...
    counter = 1L
    for(x in 1L:len){
        xUpred <- list("model"= s[[x]], "prediction"=predict(s[x], test.set))
        for(subset in 1L:counter){
            counter=counter+1L
            l[[counter]] = c(l[[subset]],xUpred)
        }
    }
  ...
  for(k in 1:p){
    ...
    minoritary <- which(df[[OUTPUT]] == min_str)
      ...
      for(conf in 1:length(configuration)){
        Ek[[length(Ek)+1]] <- configuration[[conf]](df[ind.train,], metrics, OUTPUT, CHOSEN_METRIC)
      ...
      max_set <- all_sets[[which.max(unlist(max_metric_list))]]
      cat(sprintf("Max metric %s of ensemble is %f with length %d\n", CHOSEN_METRIC, max_metric_value, length(max_set)/2))
      E[[length(E)+1]] <- max_set[seq_along(max_set) %% 2 != 0]
...
cat("######## Training mixed exhaustive IPIP ######## \n")
 for (i in 1:length(folds)) {
    ...
    end_time <- Sys.time()
    mean_time.IPIPexhaustMixed = mean_time.IPIPexhaustMixed + as.numeric(end_time - start_time)
...
matrix_mean.IPIPseqMixed <- matrix(mean_metrics.IPIPseqMixed, nrow = length(seed_algorithms), ncol = number_metrics, byrow = TRUE)
...
rownames(matrix_mean.IPIPseqMixed) <- alg_names
