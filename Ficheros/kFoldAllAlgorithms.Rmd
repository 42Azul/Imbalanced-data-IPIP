...
print(sprintf("Seed algorithm: %d", alg))
  ...
  for (i in 1:length(folds)) {
    ...
    ################# MIXED EXHAUSTIVE IPIP ################
    if(alg==1){
      print("Training mixed exhaustive IPIP")
      start_time <- Sys.time()
      
      
      ensemble.fold <- train_IPIP_exhaustive(0.55, OUTPUT_VAR, OUTPUT_MIN,
            OUTPUT_MAJ, train.set, test.set, seed_algorithms, prediction,  metrics,np, p)
      end_time <- Sys.time()
      mean_time.IPIPexhaustMixed = mean_time.IPIPexhaustMixed + end_time - start_time
      
      print(sprintf("Ensembles length in exhaustive MIXED fold: %d", i))
    ...
    print(sprintf("Ensembles length in exhaustive REPEAT fold: %d", i))
    print(unlist(lapply(ensemble.fold,length)))
    ...
    print("Training the seed algorithm")
  ...
  apply(matrix(unlist(metrics.final.IPIPexhaustRepeat), ncol= 7, byrow=T), 2, mean))
  
  
    mean_metrics.seed <- append(mean_metrics.seed, 
        apply(matrix(unlist(metrics.final.seed), ncol= 7, byrow=T), 2, mean))
    
  if(alg==1)mean_metrics.IPIPexhaustMixed <- append(mean_metrics.IPIPexhaustMixed,
  apply(matrix(unlist(metrics.final.IPIPexhaustMixed), ncol= 7, byrow=T), 2, mean))
...
matrix_mean.IPIPexhaustMixed <- matrix(mean_metrics.IPIPexhaustMixed, nrow = 1, ncol = 7, byrow = TRUE)
...
colnames(matrix_mean.IPIPexhaustMixed) <- col_names
