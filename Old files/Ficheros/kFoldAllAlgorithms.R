...
folds <-imbalancedFold(data, 5,OUTPUT_VAR, OUTPUT_MIN)
...
prediction <- function(conj.model, x, q = default_q){ #q=0.75, pero se deberían probar valores como 0.5, 0.25, 0.75...
...
train_IPIP <- function( prop.maj, OUTPUT, min_str, maj_str, train.set, test.set, 
                        configuration, prediction, metrics, b, np,  p, mt, CHOSEN_METRIC = "KAPPA"){
  ...
  for(k in 1:p){
    ...
    while(length(Ek$ensemble)<=b && i<mt(b,length(Ek$ensemble))){
      ...
      model <- configuration[[length(Ek$ensemble)+1]](df[ind.train,], metrics, OUTPUT, CHOSEN_METRIC)

      
      if (length(Ek$ensemble)==0){
          u <- -Inf;
          names(u) <- CHOSEN_METRIC;
          metrics.ensemble <- u
      }
      ...
      if(metrics.ensemble.new[CHOSEN_METRIC] <= metrics.ensemble[CHOSEN_METRIC]){ 
        i <- i+1
      } else{ 
...
train_IPIP_exhaustive <- function( prop.maj, OUTPUT, min_str, maj_str, train.set, test.set, configuration, prediction, metrics, np,  p, CHOSEN_METRIC = "KAPPA"){
  ...
  for(k in 1:p){
    ...
    train.test <- df[ind.train,]
      ...
      for(conf in 1:length(configuration)){
        ...
        Ek[[length(Ek)+1]] <- configuration[[conf]](train.test, metrics, OUTPUT, CHOSEN_METRIC)
...
for(alg in 1:length(seed_algorithms)){
    ...
    for (i in 1:length(folds)) {
        ...
        metrics.final.seed <- append( metrics.final.seed, metrics_all(data.frame(
          obs = test.set[[OUTPUT_VAR]],
          pred = predict(model_seed,test.set[,names(test.set) != OUTPUT_VAR]),
          prob= prediction.final.prob(list(list(model_seed)), test.set[,names(test.set) != OUTPUT_VAR]),
...
for(alg in 1:length(seed_algorithms)){
    ...
    for (i in 1:length(folds)) {
        ...
        metrics.final.seed_under <- append( metrics.final.seed_under, metrics_all(data.frame(
          obs = test.set[[OUTPUT_VAR]],
          pred = predict(model_seed_under,test.set[,names(test.set) != OUTPUT_VAR]),
          prob= prediction.final.prob(list(list(model_seed_under)), test.set[,names(test.set) != OUTPUT_VAR]),
          obs.prob = as.numeric(ifelse(test.set[[OUTPUT_VAR]] == OUTPUT_MAJ, 1, 0))
        )))
        cat("------------------------------------\n")
    }
...
for(alg in 1:length(seed_algorithms)){
    ...
    for (i in 1:length(folds)) {
        ...
        metrics.final.SMOTE <- append( metrics.final.SMOTE, metrics_all(data.frame(
          obs = test.set[[OUTPUT_VAR]],
          pred = predict(model_SMOTE,test.set[,names(test.set) != OUTPUT_VAR]),
          prob= prediction.final.prob(list(list(model_SMOTE)), test.set[,names(test.set) != OUTPUT_VAR]),
          obs.prob = as.numeric(ifelse(test.set[[OUTPUT_VAR]] == OUTPUT_MAJ, 1, 0))
        )))
        cat("------------------------------------\n")
    }
...
cat("######## Training mixed exhaustive IPIP ######## \n")
 for (i in 1:length(folds)) {
    ...
    ensemble.fold <- train_IPIP_exhaustive(prop.maj, OUTPUT_VAR, OUTPUT_MIN,
          OUTPUT_MAJ, train.set, test.set, seed_algorithms, prediction,  metrics,np, p, metric_optimize)
...
for(alg in 1:length(seed_algorithms)){
   ...
   for (i in 1:length(folds)) {
      ...
      ensemble.fold <- train_IPIP_exhaustive(prop.maj, OUTPUT_VAR, OUTPUT_MIN, OUTPUT_MAJ, train.set, test.set, conf, prediction,  metrics,np, p, metric_optimize)
...
for(alg in 1:length(seed_algorithms)){
   ...
   for (i in 1:length(folds)) {
      ...
      start_time <- Sys.time()
      ensemble.fold <- train_IPIP(prop.maj, OUTPUT_VAR, OUTPUT_MIN, OUTPUT_MAJ, train.set, test.set, conf, prediction,  metrics, b, np, p, mt, metric_optimize)
      end_time <- Sys.time()
    ...
    for(j in 0:nrow(matrix)){
        for (i in 0:ncol(matrix)) {
          if(j==0 && i==0){
            ...
            cat(metric_names[i])
            if(nchar(metric_names[i])<8){
              cat("\t")
            }
            if(nchar(metric_names[i])<4){
              cat("\t")
            }
          }
          else if(!is.nan(matrix[[j,i]])){
              if(matrix[[j, i]] == best_values[[i,elem]]) {
